...
#names = ["utterances", "speakers"]
#names = ["utterances", "speakers", "morphemes", "words", "uniquespeakers"]
names = ["corpus_train_nochi", "corpus_test_nochi", "corpus_dev_nochi"]
...
utterances_train_file=open((basePathOut + "corpus_train_nochi.tsv"), "r")
lines_train=utterances_train_file.readlines()
utterances_train_file.close()

utterances_dev_file=open((basePathOut + "corpus_dev_nochi.tsv"), "r")
lines_dev=utterances_dev_file.readlines()
utterances_dev_file.close()

utterances_test_file=open((basePathOut + "corpus_test_nochi.tsv"), "r")
...
def remove_child_lines(utterances_file, lines_):
        for line_ in lines_:
                 ...
                 column=line_.split("\t")
                 print(column[8])
                 if column[8] not in list_ and column[8]!="NA":
                        utterances_file.write(line_)
                        

utterances_train_file=open((basePathOut + "corpus_train_nochi.tsv"),"w")
remove_child_lines(utterances_train_file, lines_train)
utterances_train_file.close()

utterances_test_file=open((basePathOut + "corpus_test_nochi.tsv"),"w")
remove_child_lines(utterances_test_file, lines_test)
utterances_test_file.close()


utterances_dev_file=open((basePathOut + "corpus_dev_nochi.tsv"),"w")
