...
parser.add_argument("--char_embedding_size", type=int)
...
parser.add_argument("--char_noise_prob", type = float)
...
if args.load_from is not None:
   checkpoint = torch.load(args.load_from+args.language+".pth.tar")
...
for epoch in range(10000):
   ...
   while True:
    ...
    #  if counter % 20000 == 0 and epoch == 11:
      if epoch==args.epoch:
        if args.save_to is not None:
           torch.save(dict([(name, module.state_dict()) for name, module in named_modules.items()]), args.save_to+args.language+"semicolonforty.pth.tar")
...
#      break
  # if args.save_to is not None:
  #    torch.save(dict([(name, module.state_dict()) for name, module in named_modules.items()]), args.save_to+args.language+"last.pth.tar")
