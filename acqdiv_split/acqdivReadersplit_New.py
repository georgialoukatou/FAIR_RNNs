...
class AcqdivReaderOld():
   def __init__(self, split, language):
      ...
      if split == "traindev":
              self.utterances = readTSV([basePath+x for x in os.listdir(basePath) if x.startswith("corpus_traindev") and x.endswith(".tsv")])
      if split == "train":
	      self.utterances = readTSV([basePath+x for x in os.listdir(basePath) if x.startswith("corpus_train_") and x.endswith("tsv")])
      if split == "dev":
              self.utterances = readTSV([basePath+x for x in os.listdir(basePath) if x.startswith("corpus_dev") and x.endswith(".tsv")])
      if split == "test":
	      self.utterances = readTSV([basePath+x for x in os.listdir(basePath) if x.startswith("corpus_test") and x.endswith(".tsv")])
      ...
      self.UTTERANCE_COLNAME = {"Japanese" : "segmented.utterance", "Russian" : "segmented.utterance", "Inuktitut": "segmented.utterance", "Yucatec" : "segmented.utterance", "Turkish" : "segmented.utterance",  "Chintang" : "segmented.utterance", "Indonesian" : "segmented.utterance", "Sesotho" : "segmented.utterance"}[self.language]
...
#                           blankBeforeEOS=True, originalIterator=iterator, seed=random.randint(0, 1000000)):
...
class AcqdivReader():
   def __init__(self, language):
      with open("../portugueseChildes.csv", "r") as inFile:
         data = [x.strip('"').split('"\t"') for x in inFile.read().strip().split("\n")]
         self.header = data[0]
         self.header = dict(list(zip(self.header, range(len(self.header)))))
         data[1][1] == "Speaker"
         data = data[2:]
      print(self.header)
      print(len(data))
      snippets_count = int(len(data)/20)
      self.snippets = [data[x*20:(x+1)*20] for x in range(snippets_count)]
      print(len(self.snippets))
      random.Random(4656).shuffle(self.snippets) 
      self.test = self.snippets[:int(0.03*snippets_count)]
      self.dev = self.snippets[int(0.03*snippets_count):int(0.06*snippets_count)]
      self.train = self.snippets[int(0.06*snippets_count):]
      print(len(self.test), len(self.dev), len(self.train))

      # snippets of 20 successive utterances
      # 3% test, 3% valid
reader = AcqdivReader("portuguese")
