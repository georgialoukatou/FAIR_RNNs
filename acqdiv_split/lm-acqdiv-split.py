...
acqdivCorpusReaderdev = AcqdivReader("test", args.language)
...
def forward(numeric, train=True, printHere=False):
    ...
    if printHere:
       ...
       for i in range((args.sequence_length-1)-1):
          #           if boundaries_index[0] < len(boundaries[0]) and i+1 == boundaries[0][bo$
          #             boundary = True
...
for epoch in range(26):
   ...
   while True:
      ...
      #if counter % 20000 == 0 and epoch == 13:
     # if epoch ==13:
      #   if args.save_to is not None:
       #     torch.save(dict([(name, module.state_dict()) for name, module in named_modules.items()]), args.save_to+".pth.tar")
      if epoch==args.epoch:
        if args.save_to is not None:
           torch.save(dict([(name, module.state_dict()) for name, module in named_modules.items()]), args.save_to+args.language+".pth.tar")
  ...
  # else:
   #   min_loss="minimum loss=" + str(float(devLosses[-1])) +" epoch=" + str(epoch) + " args=" + str(args)
    ...
    #    if args.save_to is not None:
     #      torch.save(dict([(name, module.state_dict()) for name, module in named_modules.items()]), args.save_to+args.language+".pth.tar")   
#if epoch ==13:
    #  torch.save(dict([(name, module.state_dict()) for name, module in named_modules.items()]), args.save_to


#print(devLosses)
#print(min_loss)

#out_loss_file=open(args.out_loss_filename, "w")

#if args.out_loss_filename is not None:
#   with open(args.out_loss_filename, 'a') as out_loss_file:
